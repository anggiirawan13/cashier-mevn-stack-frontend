                ...
                <v-card-text>
                    ...
                    <v-data-table :isLoading="isLoading" :items="categories" :headers="headers" :items-per-page="10" :server-items-length="totalData" :options.sync="options" :search.sync="search" :footer-props="{
                        ...
                        <template v-slot:item.actions="{ item }">
                            <v-btn :to="`/categories/edit/${item._id}`" icon><v-icon small>mdi-pencil</v-icon></v-btn>
...
export default ({
    ...
    data() {
        return {
            ...
            headers: [
                ...
                { text: 'Category Name', value: 'title', sortable: false },
    ...
    methods: {
        getCategories() {
            ...
            this.$axios.get(`/categories?page=${page}&limit=${itemsPerPage}&search=${this.search}`)
            .then((response) => {
